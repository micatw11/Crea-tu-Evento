                                ...
                                <tr>
                                    <td style="font-family: Arial, &#039;Helvetica Neue&#039;, Helvetica, sans-serif; padding: 35px;">
                                        ...
                                        <p style="margin-top: 0; color: #74787E; font-size: 16px; line-height: 1.5em;">
                                            {{ $reserva->user->usuario->nombre }} {{ $reserva->user->usuario->apellido }} ha confirmado la adquisici&oacute;n de <b>{{ $reserva->publicacion->titulo}}</b> por un total de: ${{ $reserva->precio_total }}.
                                        </p>
                                        @if (count($reserva->rubros) > 0)
                                            <hr>
											<h5><label>Servicios y/o Productos</label></h5>
											@foreach ($reserva->rubros as $rubro)
												<li>{{$rubro->nombre}}</li>
											@endforeach
										@endif
										@if (count($reserva->articulos) > 0)
											<hr>
											<h5><label>Articulos</label></h5>
											@foreach  ($reserva->articulos as $articulo)
												<li>{{$articulo->nombre}}  x {{$articulo->pivot->cantidad}}</li>
											@endforeach
										@endif
                                        <!-- Action Button -->
